...
3. **Chase the anchor.** Look up the claim transaction by its `claimTxHash` on a public Cardano explorer, or on [andamioscan.io](https://andamioscan.io). A credential claim mints nothing. The transaction burns the course-state token and writes the credential into the recipient's Access Token global state, the `asset` named in the `evidence`. Confirm that credential claim event for the credential's `policyId` and `asset`, and that it matches the credential you are holding. Andamioscan's credential-claims endpoint (`/api/v2/events/credential-claims/claim/{claimTxHash}`) is the direct route, and it returns the claim's course id and credential hashes for exactly this comparison. Because the `policyId` is the course id, it also tells you which course, and which owner, issued the credential. This step needs no trust in Andamio at all.
...
- Its anchor is on **mainnet**, under course id `ae192632…`, recorded by claim transaction `7cb75099…` on 2026-06-17.
...
- You can confirm that claim yourself on any Cardano explorer using the course id and the transaction hash. You do not have to take Andamio's word for it.

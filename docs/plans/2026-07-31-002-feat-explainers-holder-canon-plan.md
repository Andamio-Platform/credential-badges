...
**Not in scope:**
- **The Open Graph card (`og.py`).** Its own plan. It is a 1200×630 raster, not an HTML page — no CSS, no focus states, no breakpoints — and it carries two unresolved design questions: whether a social card goes canon *paper* or canon *dark* (a white card in a feed is a materially different object), and whether it stays per-course, which the share-page plan's KTD2 answered for the page but explicitly not for the card.
